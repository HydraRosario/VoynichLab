...
const PARTICLE_LIBRARY_KEY = 'datasetcreator.particleLibrary';
const LEGACY_ATOM_LIBRARY_KEY = 'datasetcreator.atomLibrary';
...
class App {
  ...
  loadParticleLibrary() {
    try {
      const currentValue = window.localStorage.getItem(PARTICLE_LIBRARY_KEY);
      if (currentValue !== null) {
        const parsed = JSON.parse(currentValue);
        return Array.isArray(parsed) ? parsed : [];
      }

      const legacyValue = window.localStorage.getItem(LEGACY_ATOM_LIBRARY_KEY);
      if (legacyValue === null) return [];

      const legacyLibrary = JSON.parse(legacyValue);
      if (!Array.isArray(legacyLibrary)) return [];

      // Preserve the historical key and copy its value into the corrected
      // particle vocabulary. This makes the nomenclature migration reversible.
      window.localStorage.setItem(PARTICLE_LIBRARY_KEY, JSON.stringify(legacyLibrary));
  ...
  saveParticleLibrary() {
    window.localStorage.setItem(PARTICLE_LIBRARY_KEY, JSON.stringify(this.particleLibrary));
  }
