...
pub fn init_db(app_handle: &tauri::AppHandle) -> Result<Connection, Box<dyn std::error::Error>> {
    ...
    if schema_version == 2 {
        return Err(format!(
            "DatasetCreator found a historical V2 database at {}. Refusing to open it with the canonical V3 ontology. Run apps/dataset-creator/scripts/migrate-v2-to-v3.cjs against a copy, validate the report, and activate the V3 database explicitly.",
            db_path.display()
        ).into());
    }
    if schema_version > 3 {
