...
Evidence rule: every `Yes`, `No`, `Partial`, or `Unknown` must be tied to a cited source or explicitly marked as pending verification. Do not turn this into a novelty claim until Priority A sources in `research/publications/LITERATURE.md` are verified.
